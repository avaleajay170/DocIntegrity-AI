...
from students.models import HandwritingSample
    ...
    if request.method == 'POST':
        ...
        if not name or not code or not class_name:
            ...
            Subject.objects.create(
                ...
                code=code, class_name=class_name, division=division,
            )
...
def review_submission(request, submission_id):
    ...
    # Get student reference sample
    reference_sample = HandwritingSample.objects.filter(
        student=submission.student, is_active=True
    ).first()
    ...
    if request.method == 'POST':
        ...
        if status in ['verified', 'flagged', 'rejected']:
            ...
            messages.error(request, 'Please select a valid action.')
...
@teacher_required
def run_verification(request, submission_id):
    if request.method != 'POST':
        return redirect('teacher_submissions')
    ...
    if not reference_sample:
        messages.error(request, 'Student has no reference sample uploaded.')
        return redirect('review_submission', submission_id=submission_id)

    # Run verification engine
    from verification.engine import run_full_verification
    result = run_full_verification(submission, reference_sample)
